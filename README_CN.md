...
- [2026/07/12] **本仓库现在直接跟踪 InfiAgent 后端运行时，可从 PyPI 安装：** `pip install infiagent==3.12.24`。运行时包含智能体执行循环、上下文构建器、工具执行器、内置 level-0 工具集、支持多平台故障转移的 LLM 客户端、并发安全的经验存储与 SDK 入口，并附带完整测试套件。桌面客户端、marketplace 等应用层已移出本仓库；本构建的说明见 [RUNTIME.md](./RUNTIME.md)。

- [2026/07/12] **长程任务崩溃安全 + 经验存储并发安全：** 运行时全部持久化（执行栈、任务状态、经验文件）现采用原子写入（同目录临时文件 + fsync + rename），断电不再产生半截状态文件；历史损坏的状态文件会被自动隔离并提供干净的恢复路径，中断的工具调用在续跑时幂等重放。经验存储新增覆盖完整读-改-写的跨进程文件锁、uuid 条目 ID、文件 revision 与乐观并发检查（`expected_revision` / `expected_updated_at`）、task+global 双写事务（失败回滚），且只有 `status=active` 的经验会注入智能体上下文。

- [2026/07/12] **多平台模型故障转移（带缓存亲和）：** `llm_config.yaml` 的模型条目现在支持 `deployments` 列表——同一逻辑模型可挂多把密钥、多个平台。健康时流量粘在主部署上以保住服务端 prompt cache；限流、超时、鉴权失败会让该部署进入冷却并自动切换下一个，且存在备用部署时鉴权错误不再中止重试。经典单密钥配置完全兼容。
