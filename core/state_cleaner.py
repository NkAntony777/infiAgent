...
def clean_before_start(task_id: str, new_user_input: str = None):
    ...
    try:
        ...
        with hierarchy_manager._state_lock():
            ...
            current_agents = current["agents_status"]
            ...
            # 检查用户输入是否改变
            last_instruction = current.get("instructions", [])
            is_same_task = False
            ...
            if not is_same_task:
                # 新任务：清空 current
                current["agents_status"] = {}
                current["hierarchy"] = {}
                current["instructions"] = []
                # 删除压缩的历史（如果有）
                if "_compressed_user_agent_history" in current:
                    del current["_compressed_user_agent_history"]
                if "_compressed_user_agent_history_meta" in current:
                    del current["_compressed_user_agent_history_meta"]
                # 删除所有agent的结构化调用信息压缩缓存
                keys_to_delete = [k for k in current.keys() if k.startswith("_compressed_structured_call_info_")]
                for key in keys_to_delete:
                    del current[key]
                safe_print(f"   🗑️ 清空 current，准备新任务")
            else:
                # 续跑：保留 running agents
                current["agents_status"] = {**completed_agents, **running_agents}
                # hierarchy 保留所有
