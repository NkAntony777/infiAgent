...
class CheapClawServiceTests(unittest.TestCase):
    ...
    def _cleanup_temp_dir(self):
        # Windows 上后台任务进程可能仍短暂持有 launched_tasks 日志句柄，
        # 立即删除会触发 WinError 32；带重试等待子进程退出后再清理。
        last_error = None
        for _ in range(30):
            try:
        ...
        if last_error is not None:
            warnings.warn(f"could not fully remove temp dir {self.temp_dir.name}: {last_error}", ResourceWarning)
    ...
    def test_runtime_registry_loads_cheapclaw_custom_tools_from_dynamic_tools_dir(self):
        ...
        # 3.12.24 运行时将自定义工具改为 ExternalPythonToolProxy 代理加载，
        # source 标签从 "custom" 演进为 "custom_external"，两者均表示动态工具目录加载成功。
        self.assertIn(metadata["cheapclaw_start_task"]["source"], {"custom", "custom_external"})
    ...
    def test_start_task_binds_panel_entry(self):
        ...
        try:
            ...
            # POSIX 上可 waitpid 收尸；Windows 没有 waitpid/WNOHANG，
            # 轮询探测进程是否已退出即可（句柄由 cleanup 重试兜底）。
